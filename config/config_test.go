...
CODE_ODESSEY_DATABASE_URI=postgres://user:password@localhost:5432/codeodessey_db?sslmode=disable`
	...
	require.Equal(t, "postgres://user:password@localhost:5432/codeodessey_db?sslmode=disable", config.DBSource)

	// Set environment variables to override the values in the .env file
	os.Setenv("CODE_ODESSEY_DB_DRIVER", "mysql")
	os.Setenv("CODE_ODESSEY_DATABASE_URI", "mysql://user:password@localhost:3306/codeodessey_db")
...
CODE_ODESSEY_DATABASE_URI=postgres://user:password@localhost:5432/codeodessey_db?sslmode=disable`
	...
	require.Equal(t, "mysql://user:password@localhost:3306/codeodessey_db", config.DBSource)
}
